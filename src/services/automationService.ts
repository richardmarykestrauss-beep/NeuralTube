...
export const runAutonomousScan = async (niche: string, authorUid: string, channelId?: string | null) => {
  try {
...
export const publishVideo = async (videoId: string, title: string, channelId?: string | null) => {
  try {
    await updateDoc(doc(db, "videos", videoId), {
      stage: 'publish',
      progress: 90,
    ...
    await simulateVideoUpload(videoId, title, channelId);
  } catch (error) {
...
export const simulateVideoUpload = async (videoId: string, title: string, channelId?: string | null) => {
  try {
    const user = auth.currentUser;
    if (!user) throw new Error("User not authenticated");

    // Try channel-specific tokens first, fall back to legacy user-level tokens
    let tokens: any = null;
    if (channelId) {
      const chDoc = await getDoc(doc(db, "users", user.uid, "channels", channelId));
      tokens = chDoc.data()?.youtubeTokens;
    }
    if (!tokens) {
      const userDoc = await getDoc(doc(db, "users", user.uid));
      tokens = userDoc.data()?.youtubeTokens;
    }
    ...
    if (!tokens) {
      ...
      toast.error("YouTube not connected. Go to My Channels to connect a channel.");
      return;
    }
